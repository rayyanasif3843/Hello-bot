...
async def warnings(ctx, member: discord.Member):
    ...
    if not user_warns:
        return await ctx.send(
            embed=quick_embed(
                "ℹ️ Warnings",
                "No warnings found for this member.",
...
async def slash_snipe(interaction: discord.Interaction):
    ...
    if not data:
        return await interaction.response.send_message(
            embed=quick_embed("📭 Snipe", "Nothing to snipe.", discord.Color.light_grey())
        )
    embed = discord.Embed(title="🎯 Sniped Message", color=discord.Color.blurple())
    embed.add_field(name="Author", value=data["author"], inline=False)
    embed.add_field(name="Message", value=data["content"], inline=False)
...
async def userinfo(ctx, member: discord.Member = None):
    ...
    embed.add_field(name="Joined", value=str(member.joined_at) if member.joined_at else "Unknown")
...
class ApplicationDropdown(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            await user.send(embed=quick_embed("Moderator Application", "Please answer all questions."))
            ...
            embed.add_field(name="Time Taken", value=f"{round(end - start)} seconds", inline=False)

            channel = bot.get_channel(APPLICATION_CHANNEL_ID)
            if channel:
                await channel.send(embed=embed, view=ApplicationDecisionView(user.id))
